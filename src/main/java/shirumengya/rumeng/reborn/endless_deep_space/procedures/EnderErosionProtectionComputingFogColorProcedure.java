...
public class EnderErosionProtectionComputingFogColorProcedure {
	@OnlyIn(Dist.CLIENT)
	@SubscribeEvent
	...
	private static void execute(@Nullable Event event, Entity entity, ViewportEvent.ComputeFogColor fogcolor) {
		if (entity == null || fogcolor == null)
			return;
		if (entity instanceof LivingEntity _livEnt ? _livEnt.hasEffect(EndlessDeepSpaceModMobEffects.ENDER_EROSION_PROTECTION.get()) : false) {
			fogcolor.setRed(85 / 255.0F);
			fogcolor.setGreen(239 / 255.0F);
			fogcolor.setBlue(240 / 255.0F);
		}
	}
}
