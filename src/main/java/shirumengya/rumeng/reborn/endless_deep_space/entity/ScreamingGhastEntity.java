...
public class ScreamingGhastEntity extends Monster {
	...
	public final CustomServerBossEvent bossInfo = new CustomServerBossEvent(this, this.getDisplayName(), CustomServerBossEvent.BossBarColor.WHITE, CustomServerBossEvent.BossBarOverlay.PROGRESS);
	...
	private static final Predicate<LivingEntity> LIVING_ENTITY_SELECTOR = (p_31504_) -> {
   	...
   	private static final EntityDataAccessor<Boolean> DataSummoned = SynchedEntityData.defineId(ScreamingGhastEntity.class, EntityDataSerializers.BOOLEAN);
   ...
   protected SoundEvent getAmbientSound() {
	...
	public boolean hurt(@Nullable ScreamingGhastPart part, DamageSource source, float amount) {
		...
		(amount >= this.getMaxHealth() && !(source.getEntity() instanceof Player))) {
			return false;
		} else {
			if (!(source.getEntity() instanceof Player player && player.getAbilities().instabuild) && source.getEntity() != this.getTarget()) {
				return false;
			}
			...
			return super.hurt(source, amount2);
		}
	}
	...
	public void tick() {
		...
		if (!this.level.isClientSide && !this.entityData.get(DataSummoned)) {
			ModMessages.sendToAllPlayers(new SendEndlessDeepSpaceCommonToastS2CPacket(new ItemStack(EndlessDeepSpaceModItems.SCREAMING_GHAST_SPAWN_EGG.get()), this.getDisplayName(), Component.translatable("endless_deep_space.boss.spawning", this.getDisplayName()), 5000L, -11534256, -16777216));
